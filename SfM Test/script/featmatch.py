...
def FeatMatch(opts, data_files=[]): 
    ...
    for i, img_path in enumerate(img_paths): 
        ...
        # Modified to handle different feature detectors
        if opts.features in ['SIFT', 'SURF']:
            # Use xfeatures2d module for SIFT and SURF
    ...
    num_done = 0 
    num_matches = ((len(img_paths)-1) * (len(img_paths))) // 2  # Integer division with //
...
def SetArguments(parser): 
    ...
    parser.add_argument('--features',action='store', type=str, default='SIFT', dest='features',
                        help='[SIFT|SURF|ORB|BRISK|AKAZE] Feature algorithm to use (default: SIFT)') 
