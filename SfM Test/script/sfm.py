...
class SFM(object): 
    ...
    def _LoadFeatures(self, name): 
        # Changed 'r' to 'rb' for binary mode
    ...
    def _BaselinePoseEstimation(self, name1, name2):
        ...
        # Modified to use compatible API for findFundamentalMat in OpenCV 4.11.0
        # Using cv2.RANSAC directly instead of opts.fund_method
        # and changing parameter names to match the current API
        F, mask = cv2.findFundamentalMat(
            img1pts, 
            img2pts, 
            cv2.RANSAC,  # Fixed method
            self.opts.outlier_thres,  # ransacReprojThreshold
            self.opts.fund_prob  # confidence
        )
        ...
        for prev_name in self.image_data.keys(): 
            if prev_name != name: 
                ...
                if len(matches) > 0: 
                    ...
                    # Modified to use compatible API for findFundamentalMat
                    F, mask = cv2.findFundamentalMat(
                        img1pts, 
                        img2pts, 
                        cv2.RANSAC,  # Fixed method
    ...
    def _NewViewPoseEstimation(self, name): 
        ...
        # Modified to use compatible API for solvePnPRansac
...
def SetArguments(parser): 
    ...
    parser.add_argument('--dataset',action='store',type=str,default='castle-P19',dest='dataset',
                        help='name of dataset (default: castle-P19)') 
...
def PostprocessArgs(opts): 
    # Removed converting opts.fund_method to a cv2 attribute since we're using cv2.RANSAC directly
